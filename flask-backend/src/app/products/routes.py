...
@products_bp.post("/<uuid:market_uuid>/bulk-upload")
...
def bulk_upload_products(market_uuid, files_data):
    product_file = files_data["product_file"]
    result = product_service.process_bulk_file(str(market_uuid), product_file)
    return result
